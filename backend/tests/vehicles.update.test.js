...
describe('PUT /api/vehicles/:id', () => {
  ...
  it('should allow an admin to update a vehicle and return 200', async () => {
    const updates = { price: 26500, quantity: 8 };
    ...
    expect(response.body.vehicle.price).toBe(26500);
    expect(response.body.vehicle.quantity).toBe(8);
    expect(response.body.vehicle.make).toBe(seedVehicle.make);
  });
  ...
  it('should return 400 when update data fails validation', async () => {
    const response = await request(app)
      .put(`/api/vehicles/${vehicleId}`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ price: -100 });
